...
rbind(
  ...
  mutate(percent = nb/total*100,
    ...
    geom_bar(aes(fill = factor(scope, levels = c("Tous BV", "QPV"))), stat = "identity", position = position_dodge(), width = .6, color = "black") +
    labs(x = "",
...
theme_acp_1.2 <- function(plot){
    plot +
    geom_label(aes(x = 1.15, y = 0), hjust = 0.5, label = "Abstention", font = "Helvetica", label.size = NA, size = 4) +
    geom_label(aes(x = -1.15, y = 0), hjust = 0.5, label = "Droite macroniste", font = "Helvetica", label.size = NA, size = 4) +
    geom_label(aes(x = 0, y = 1.05), hjust = 0, label = "Extrême \ndroite", font = "Helvetica", label.size = NA, size = 4) +
    geom_label(aes(x = 0, y = -1.05), hjust = 1, label = "Extrême \ngauche", font = "Helvetica", label.size = NA, size = 4) +
    xlim(-1.2, 1.2) +
    ylim(-1.4, 1.4) + 
    coord_flip() +
    labs(x = "Dimension 1 (29.1%)", y = "Dimension 2 (19.1%)",
         title = "Projection des variables sur les deux premières dimensions")
}
```
