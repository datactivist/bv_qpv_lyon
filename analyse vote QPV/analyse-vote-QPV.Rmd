...
graph <- left_join(ensemble |> select(type) |> rownames_to_column(),
    ...
    geom_label(aes(x = 13, y = 0), hjust = 0.5, label = "Abstention", label.size = NA, size = 4) +
    geom_label(aes(x = -11, y = 0), hjust = 0.5, label = "Droite et Macron", label.size = NA, size = 4) +
    geom_label(aes(x = 0, y = 9), hjust = 0, label = "Extrême \ndroite", label.size = NA, size = 4) +
    geom_label(aes(x = 0, y = -9), hjust = 1, label = "Gauche", label.size = NA, size = 4) +
    xlim(-12, 13.5) +
    ylim(-11, 12) + 
    coord_flip() +
    scale_x_continuous(trans = "reverse") +
