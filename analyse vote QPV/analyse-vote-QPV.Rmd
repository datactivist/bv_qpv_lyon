...
rbind(
  ...
  mutate(percent = nb/total*100,
    ...
    # geom_text(aes(y = percent + 0.1*max(percent), label = percent_lbl), 
...
as.data.frame(res.desc$Dim.1) |> rownames_to_column() |> na.omit() |> gt::gt() |> gt::fmt_number(decimals = 2)
```
...
```{r dimdesc.2}
as.data.frame(res.desc$Dim.2) |> rownames_to_column() |> na.omit() |> gt::gt() |> gt::fmt_number(decimals = 2)
```

##### Dimension 3

```{r dimdesc.3}
as.data.frame(res.desc$Dim.3) |> rownames_to_column() |> na.omit() |> gt::gt() |> gt::fmt_number(decimals = 2)
```
