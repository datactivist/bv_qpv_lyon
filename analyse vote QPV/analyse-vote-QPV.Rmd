...
metropole <- st_read("https://data.grandlyon.com/geoserver/metropole-de-lyon/ows?SERVICE=WFS&VERSION=2.0.0&request=GetFeature&typename=metropole-de-lyon:adr_voie_lieu.adrmetropole&outputFormat=application/json&SRSNAME=EPSG:4171&sortBy=gid", quiet = TRUE)
    ...
    rbind(qpv_l17_t1 |> 
      ...
      qpv_p17_t1 |> 
        left_join(pop_qpv |> select(codeQPV, popMuniQPV),
                          by = c("CODE_QP" = "codeQPV")) |> 
        select(CODE_QP, popMuniQPV, Inscrits, Votants) |> 
        mutate(type = "Présidentielle 2017 tour 1") |> 
        rename(id = CODE_QP, Population = popMuniQPV),
      qpv_p22_t1 |> 
        left_join(pop_qpv |> select(codeQPV, popMuniQPV),
                          by = c("CODE_QP" = "codeQPV")) |> 
        select(CODE_QP, popMuniQPV, Inscrits, Votants) |> 
        mutate(type = "Présidentielle 2022 tour 1") |> 
        rename(id = CODE_QP, Population = popMuniQPV),
      qpv_p17_t2 |> 
        left_join(pop_qpv |> select(codeQPV, popMuniQPV),
                          by = c("CODE_QP" = "codeQPV")) |> 
        select(CODE_QP, popMuniQPV, Inscrits, Votants) |> 
        mutate(type = "Présidentielle 2017 tour 2") |> 
        rename(id = CODE_QP, Population = popMuniQPV),
      qpv_p22_t2 |> 
        left_join(pop_qpv |> select(codeQPV, popMuniQPV),
                          by = c("CODE_QP" = "codeQPV")) |> 
        select(CODE_QP, popMuniQPV, Inscrits, Votants) |> 
        mutate(type = "Présidentielle 2022 tour 2") |> 
        rename(id = CODE_QP, Population = popMuniQPV)) |> 
  group_by(type) |> 
  ...
  mutate(percent = nb/total*100,
      ...
      rbind(bv_l17_t1 |> 
          ...
          bv_l22_t1 |> 
            ...
            select(code_insee, Population, Inscrits, Votants, type),
          bv_p17_t1 |> 
            mutate(type = "Présidentielle 2017 tour 1", Population = 1386000) |> 
            select(code_insee, Population, Inscrits, Votants, type),
          bv_p22_t1 |> 
            mutate(type = "Présidentielle 2022 tour 1", Population = 1411570) |> 
            select(code_insee, Population, Inscrits, Votants, type),
          bv_p17_t2 |> 
            mutate(type = "Présidentielle 2017 tour 2", Population = 1386000) |> 
            select(code_insee, Population, Inscrits, Votants, type),
          bv_p22_t2 |> 
            mutate(type = "Présidentielle 2022 tour 2", Population = 1411570) |> 
            select(code_insee, Population, Inscrits, Votants, type)) |> 
      group_by(type) |> 
      ...
      mutate(percent = nb/total*100,
         percent_lbl = paste0(round(percent), " %"),
         compte = fct_reorder(compte, nb, .desc = TRUE),
         scope = "Tous les bureaux de vote")
) |>
    ggplot(aes(x = compte, y = percent)) +
    geom_bar(aes(fill = factor(scope, levels = c("Tous les bureaux de vote", "QPV"))), stat = "identity", position = position_dodge(), width = .6) +
    labs(x = "",
    ...
    scale_fill_manual(values = c("Tous les bureaux de vote" = "wheat2", "QPV" = "brown3")) +
    facet_wrap(~type, ncol = 2, scales='free') +
    scale_y_continuous(labels = scales::comma) +
    labs(fill = "")
graph
saving_plot(graph, "population_inscrits_votants", 7, 9)
ggsave(file = "Save_graphs/population_inscrits_votants.svg", plot=graph, width=7, height=9)
```
...
qpv_demo <- tribble(
    ...
    "QPV",               0.265        , 0.141         , 0.414         , 0.118         , 0.063        , 0.263                     
)
...
natio_metro <- natio_metro |>
    ...
    group_by(nat) %>% 
    summarise(count = sum(count)) |>
    mutate(etrangers = count / sum(count)) |>
...
violinplot_comp <- function(bv, qpv, elections, interpretation, min_x, max_x){
    ...
    rbind(bv_all, bv_hors_qpv, qpv)|> 
        ...
        mutate(percent_inscrits = nb_pers/Inscrits*100,
               ...
               vote = str_replace_all(vote, c("Macron" = "E. Macron",
                                              "Fillon" = "F.Fillon",
                                              "Mélenchon" = "JL. Mélenchon",
                                              "Le Pen" = "M. Le Pen",
                                              "Hamon" = "B. Hamon",
                                              "Dupont Aignan" = "N. Dupont-Aignan",
                                              "Asselineau" = "F. Asselineau",
                                              "Poutou" = "P. Poutou",
                                              "Lassalle" = "J. Lassalle",
                                              "Arthaud" = "N. Arthaud",
                                              "Cheminade" = "J. Cheminade",
                                              "Zemmour" = "E. Zemmour",
                                              "Jadot" = "Y. Jadot",
                                              "Pécresse" = "V. Pécresse",
                                              "Hidalgo" = "A. Hidalgo",
...
barplot_comp <- function(bv, qpv, elections, interpretation, min_x, max_x, facteurs = rev(c("Abstentions", "Blancs", "Nuls", "ARTHAUD", "CHEMINADE", "POUTOU", "MÉLENCHON", "HAMON", "MACRON", "FILLON", "LASSALLE", "DUPONT AIGNAN", "ASSELINEAU", "LE PEN"))){
    common_cols <- intersect(colnames(bv), colnames(qpv))
    bv <- bv |> 
    ...
    bind_rows(bv_all, qpv)|> 
        ...
        mutate(percent_inscrits = nb_pers/Inscrits*100,
               ...
               vote = str_replace_all(vote, c("Macron" = "E. Macron",
                                              ...
                                              "Hidalgo" = "A. Hidalgo",
                                              "Roussel" = "F. Roussel")),
               vote = fct_relevel(vote, facteurs)) |> 
        filter(!vote %in% c("Votants", "Exprimés")) |>
        ggplot(aes(x = vote, y = percent_inscrits, fill = type)) +
            ...
            labs(title = interpretation, 
                 subtitle = paste0("Comparaison des votes en % des inscrits entre l'ensemble de la métropole et \nles QPV ", elections), 
...
```{r fig.height=10}
graph <- barplot_comp(bv_p17_t2, qpv_p17_t2, "au second tour des présidentielles de 2017", "L'abstention est beaucoup plus élevée dans les QPV,\net le vote pour Macron plus faible.", 0, 60, facteurs = rev(c("Abstentions", "Blancs", "Nuls", "MACRON","LE PEN")))
graph
saving_plot(graph, "bar_p17_t2", 7, 10)
```

### Pre.22 tour 1

```{r fig.height=10}
graph <- barplot_comp(bv_p22_t1, qpv_p22_t1, "au premier tour de l'élection présidentielle de 2022", "L'abstention est beaucoup plus élevée dans les QPV, et le vote pour Mélenchon également. \nTous les autres votes y sont plus faibles.", 0, 40, facteurs = rev(c("Abstentions", "Blancs", "Nuls", "ARTHAUD","POUTOU", "MÉLENCHON", "ROUSSEL", "HIDALGO", "JADOT", "MACRON", "PÉCRESSE", "LASSALLE", "DUPONT AIGNAN", "LE PEN", "ZEMMOUR")))
graph
saving_plot(graph, "bar_p22_t1", 7, 10)
```

### Pre.22 tour 2

```{r fig.height=10}
graph <- barplot_comp(bv_p22_t2, qpv_p22_t2, "au second tour de l'élection présidentielle de 2022", "L'abstention est beaucoup plus élevée dans les QPV. Le vote pour Macron est inférieur à ce qui est observé dans le reste de la Métropole.", 0, 60, facteurs = rev(c("Abstentions", "Blancs", "Nuls", "MACRON", "LE PEN")))
graph
saving_plot(graph, "bar_p22_t2", 7, 10)
```
...
ensemble <- rbind(var_acp |> mutate(type = "Tous les bureaux de vote"),
...
graph <- left_join(ensemble |> select(type) |> rownames_to_column(),
    ...
    theme_minimal() +
    scale_color_manual(name = "", values = c("Tous les bureaux de vote" = "wheat2", "QPV" = "brown3")) +
    scale_fill_manual(name = "", values = c("Tous les bureaux de vote" = "wheat2", "QPV" = "brown3")) +
    ...
    scale_x_continuous(trans = "reverse") +
    labs(x = "Dimension 1 (29.1%)", y = "Dimension 2 (19.1%)",
         title = "Projection des bureaux de vote et QPV sur les deux premières dimensions") +
    guides(fill = "none", shape = "none", alpha = "none", 
           color = guide_legend(label.vjust = 1)) 
ggplty <- ggplotly(graph, tooltip = "text")

for (i in 1:length(ggplty$x$data)){
    if (!is.null(ggplty$x$data[[i]]$name)){
        ggplty$x$data[[i]]$name =  gsub("\\(","",str_split(ggplty$x$data[[i]]$name,",")[[1]][1])
    }
}
