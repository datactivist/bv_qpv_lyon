...
# communes_gd_lyon <- read_csv2("https://download.data.grandlyon.com/ws/grandlyon/adr_voie_lieu.adrcomgl/all.csv?maxfeatures=-1&start=1")
# 
...
communes_gd_lyon_shp <- st_read(here("data", "adr_voie_lieu.adrcomgl.geojson"), drivers = "GeoJSON", quiet = TRUE)
communes_gd_lyon <- communes_gd_lyon_shp %>% st_drop_geometry() %>% select(nom, nomreduit, insee) 
```
...
# liste_2022 <- read_csv(here("data/listes électorales", "liste_2022_dep69.csv"))
```
...
poids_2022 <- poids_2022 %>% 
    ...
    st_as_sf(coords = c("longitude", "latitude")) %>% 
    st_set_crs(4326)
st_write(poids_2022, here("output_data", "poids_2022.gpkg"))
```
...
header_L17T1 <- names(read_csv2(path_leg_2017_t1, n_max = 1))
...
legislatives_2017_t1_bv <- read_csv2(here("data", "legislatives_2017_T1_bv.csv"), locale = locale(encoding = "ISO-8859-1"))
names(legislatives_2017_t1_bv) <- header_L17T1
...
# bv_grandlyon_p2017_t1_res <- st_read(here("output_data", "bv_grandlyon_p2017_t1_res.gpkg"))
...
bv_grandlyon_p2017_t2_res <- bv_2017 %>% 
  ...
  filter(!is.na(Inscrits))
st_write(bv_grandlyon_p2017_t2_res, here("output_data", "bv_grandlyon_p2017_t2_res.gpkg"), append = FALSE)
# bv_grandlyon_p2017_t2_res <- st_read(here("output_data", "bv_grandlyon_p2017_t2_res.gpkg"))

bv_grandlyon_l2017_t1_res <- bv_2017 %>% 
  mutate(bureau_vote_id = str_replace(bureau_vote_id, "6938[1-9]", "69123")) %>% 
  left_join(leg_2017_R1_BV_cleaned, by = join_by(bureau_vote_id == CodeBV)) %>% 
  filter(!is.na(Inscrits))
st_write(bv_grandlyon_l2017_t1_res, here("output_data", "bv_grandlyon_l2017_t1_res.gpkg"), append = FALSE)
...
bv_grandlyon_p2022_t1_res <- bv_communes_2022 %>% 
  ...
  filter(!is.na(Inscrits))
st_write(bv_grandlyon_p2022_t1_res, here("output_data", "bv_grandlyon_p2022_t1_res.gpkg"), append = FALSE)

bv_grandlyon_p2022_t2_res <- bv_communes_2022 %>% 
  mutate(bureau_vote_id = str_replace(bureau_vote_id, "6938[1-9]", "69123")) %>% 
  left_join(presidentielle_2022_t2_bv_cleaned, by = join_by(bureau_vote_id == CodeBV)) %>% 
  filter(!is.na(Inscrits))
st_write(bv_grandlyon_p2022_t2_res, here("output_data", "bv_grandlyon_p2022_t2_res.gpkg"), append = FALSE)

bv_grandlyon_l2022_t1_res <- bv_communes_2022 %>% 
  mutate(bureau_vote_id = str_replace(bureau_vote_id, "6938[1-9]", "69123")) %>% 
  left_join(leg_2022_R1_BV_cleaned %>% mutate(CodeBV = str_replace(CodeBV, "6938[1-9]", "69123")), by = join_by(bureau_vote_id == CodeBV)) %>% 
  filter(!is.na(Inscrits))
st_write(bv_grandlyon_l2022_t1_res, here("output_data", "bv_grandlyon_l2022_t1_res.gpkg"), append = FALSE)
# bv_grandlyon_l2022_t1_res <- st_read(here("output_data", "bv_grandlyon_l2022_t1_res.gpkg"))
...
# qpv69_pres_17_t1 <- st_read(here("output_data", "qpv69_pres_17_t1.gpkg"))
...
# qpv69_pres_17_t2 <- st_read(here("output_data", "qpv69_pres_17_t2.gpkg"))
...
# qpv69_leg_17_t1 <- st_read(here("output_data", "qpv69_leg_17_t1.gpkg"))
...
qpv69_leg_22_t1 <- tidycensus::interpolate_pw(from = bv_grandlyon_l2022_t1_res %>% select(-contains(".ins"), -contains("_ins"),  -contains(".exp"), -contains("_vot"), -contains("NbCand")), to = qpv_gd_lyon, to_id = "CODE_QP", extensive = TRUE, weights = poids_2022, weight_column = "n")
st_write(qpv69_leg_22_t1, here("output_data", "qpv69_leg_22_t1.gpkg"))
# qpv69_leg_22_t1 <- st_read(here("output_data", "qpv69_leg_22_t1.gpkg"))
